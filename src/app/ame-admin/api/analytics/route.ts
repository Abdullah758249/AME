...
export async function GET(request: NextRequest) {
  ...
  const [todayCount, weekCount, monthCount, totalCount] = await Promise.all([
    prisma.pageVisit.count({ where: { createdAt: { gte: subDays(now, 1) } } }),
    prisma.pageVisit.count({ where: { createdAt: { gte: subWeeks(now, 1) } } }),
    prisma.pageVisit.count({ where: { createdAt: { gte: subMonths(now, 1) } } }),
    prisma.pageVisit.count(),
  ]);

  const topPages = await prisma.pageVisit.groupBy({
    by: ["path"], where, _count: { id: true }, orderBy: { _count: { id: "desc" } }, take: 10,
  });
  const sources = await prisma.pageVisit.groupBy({
    by: ["source"], where, _count: { id: true }, orderBy: { _count: { id: "desc" } }, take: 10,
  });
  const countries = await prisma.pageVisit.groupBy({
    by: ["country"], where, _count: { id: true }, orderBy: { _count: { id: "desc" } }, take: 20,
  });
