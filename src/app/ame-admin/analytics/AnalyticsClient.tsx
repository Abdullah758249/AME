...
// تعريف الأنواع (نفس ما كان موجودًا)
interface Stat {
  label: string;
  value: number;
...
export function AnalyticsClient({ initialStats }: { initialStats: AnalyticsData }) {
  const [period, setPeriod] = useState<"today" | "week" | "month" | "all">("all");
  const [data, setData] = useState<AnalyticsData>(initialStats);
  const [loading, setLoading] = useState(false);

  // جلب البيانات عند تغيير الفلتر
  const fetchData = async (newPeriod: typeof period) => {
    setLoading(true);
    try {
      const res = await fetch(`/ame-admin/api/analytics?period=${newPeriod}`);
  ...
  return (
    <>
      ...
      {/* الدول مع الرسم البياني الدائري */}
